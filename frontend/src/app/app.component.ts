...
export class Player {
	...
	attack(cardId: string, targetId: string, hero: string) {
			...
			playReceived(gameId: string, e?: Entity): void {
				...
				if(e) {
					...
					if(e == this.owner.hero) {
						...
						AppComponent.stompClient.send(`/app/game/${gameId}/castTargetedSpell`, {}, JSON.stringify({own: "true", hero: "true", cardId: this.id, targetId: null}));
					} else if(e == this.owner.opponent.hero) {
						console.log("Envoi de castTargetedSpell sur le hero adverse");
						AppComponent.stompClient.send(`/app/game/${gameId}/castTargetedSpell`, {}, JSON.stringify({own: "false", hero: "true", cardId: this.id, targetId: null}));
					} else {
						...
						try {
							...
							if(this.owner.board.get(targetId)) {
								...
								AppComponent.stompClient.send(`/app/game/${gameId}/castTargetedSpell`, {}, JSON.stringify({own: "true", hero: "false", cardId: this.id, targetId: targetId}));
							} else if(this.owner.opponent.board.get(targetId)) {
								console.log("Envoi de castTargetedSpell de " + this.name + " sur le board adverse");
								AppComponent.stompClient.send(`/app/game/${gameId}/castTargetedSpell`, {}, JSON.stringify({own: "false", hero: "false", cardId: this.id, targetId: targetId}));
							} else {
		...
		export class HeroMage extends Hero {
			...
			specialReceived(gameId: string, e: Entity) {
				...
				if(e == this.owner.hero) {
					...
					AppComponent.stompClient.send(`/app/game/${gameId}/heroTargetedSpecial`, {}, JSON.stringify({own: "true", hero: "true", targetId: null}));
				} else if(e == this.owner.opponent.hero) {
					console.log("Envoi de castTargetedSpecial sur le hero adverse");
					AppComponent.stompClient.send(`/app/game/${gameId}/heroTargetedSpecial`, {}, JSON.stringify({own: "false", hero: "true", targetId: null}));
				} else {
					...
					try {
						...
						if(this.owner.board.get(targetId)) {
							...
							AppComponent.stompClient.send(`/app/game/${gameId}/heroTargetedSpecial`, {}, JSON.stringify({own: "true", hero: "false", targetId: targetId}));
						} else if(this.owner.opponent.board.get(targetId)) {
							console.log("Envoi de castTargetedSpecial de mage sur le board adverse");
							AppComponent.stompClient.send(`/app/game/${gameId}/heroTargetedSpecial`, {}, JSON.stringify({own: "false", hero: "false", targetId: targetId}));
						} else {
							console.log("Carte cible non trouvée");
						}
					} catch {
